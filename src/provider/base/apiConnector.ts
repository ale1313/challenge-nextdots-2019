...
export default class APIConnector {
  ...
  _request(uri, args = {} as any) {
    ...
    return new Promise((resolve, reject) => {
      let request: any = fetch(uri, options);
      let timeoutReached: boolean = false;
      let requestDone: boolean = false;

      if (this._timeout) {
        setTimeout(() => {
          if (requestDone) return;
          timeoutReached = true;
          let err: any = new TypeError(Errors.TIMEOUT_MSG);
